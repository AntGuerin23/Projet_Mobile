...
class ProfileFragment : Fragment(R.layout.fragment_profile), AdapterView.OnItemSelectedListener {
    ...
    private fun initializeFields(view: View, spinner: Spinner) {
        view.findViewById<TextView>(R.id.tvUserName).text = User.firstname + " " + User.lastname
        ...
        val startForResult =
            registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result: ActivityResult ->
                if (result.resultCode == Activity.RESULT_OK) {
                    ...
                    val statement: PreparedStatement = Database.connectDB()!!
                        .prepareStatement("UPDATE users SET picture = ? WHERE email = ?")
